# **Brain Metastasis Segmentation (BraTS-MET)**

This tool performs Pre- and Post-Treatment Brain Metastases segmentation (BraTS-MET).
...
3. Pull the prebuilt Docker image from the EUCAIM registry:
    ...
    ```bash
    docker pull harbor.eucaim.cancerimage.eu/processing-tools/brain-metastasis-segmenter:v1.0.0
    ```
...
docker run --rm --network none --gpus=all --memory=16G --shm-size 4G \
        ...
        harbor.eucaim.cancerimage.eu/processing-tools/brain-metastasis-segmenter:v1.0.0
```
