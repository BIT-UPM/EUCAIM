# **Pediatric Brain Tumor Segmentation (BraTS-PED)**

This tool performs Pre-Treatment Pediatric Tumor Patients segmentation (BraTS-PED).
...
3. Pull the prebuilt Docker image from the EUCAIM registry:
    ...
    ```bash
    docker pull harbor.eucaim.cancerimage.eu/processing-tools/pediatric-brain-tumor-segmenter:v1.0.0
    ```
...
docker run --rm --network none --gpus=all --memory=16G --shm-size 4G \
        ...
        harbor.eucaim.cancerimage.eu/processing-tools/pediatric-brain-tumor-segmenter:v1.0.0
```
