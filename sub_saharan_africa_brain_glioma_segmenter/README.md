# **Sub-Saharan Africa Brain Glioma Segmentation (BraTS-SSA)**

This tool performs Brain Glioma segmentation in the underserved sub-Saharan African patient population (BraTS-SSA).
...
3. Pull the prebuilt Docker image from the EUCAIM registry:
    ...
    ```bash
    docker pull harbor.eucaim.cancerimage.eu/processing-tools/ssa-brain-glioma-segmenter:v1.0.0
    ```
...
docker run --rm --network none --gpus=all --memory=16G --shm-size 4G \
        ...
        harbor.eucaim.cancerimage.eu/processing-tools/ssa-brain-glioma-segmenter:v1.0.0
```
...
- Parida, A., Capellán-Martín, D., Jiang, Z., Tapp, A., Liu, X., Anwar, S. M., ... & Linguraru, M. G. (2024). Adult glioma segmentation in sub-saharan africa using transfer learning on stratified finetuning data. arXiv preprint arXiv:2412.04111. https://doi.org/10.48550/arXiv.2412.04111
